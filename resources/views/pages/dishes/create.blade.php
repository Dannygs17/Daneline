...
@section('title', 'Agregar platillo')
		...
		<div class="card-body p-5">
			<form method="post" action="{{ route('dishes.store') }}" enctype="multipart/form-data">
					...
					<select name="category" id="category" class="form-select @error('category') is-invalid @enderror">
						@foreach(\App\Enums\DishCategory::cases() as $cat)
							<option value="{{ $cat->value }}"
							        @selected(old('category', $category ?? '') == $cat->value)>
				...
				<div class="d-grid gap-2">
					...
					<a href="{{ route('dishes.get-by-category', $category ?? 'appetizer') }}" class="btn btn-outline-secondary">Cancelar</a>
				</div>
			</form>
		</div>
	</div>
